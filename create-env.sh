...
echo -e "Creating 3 new instances \e[0m"
aws ec2 run-instances --image-id $1 --key-name awskey --security-group-ids sg-d5539bac --instance-type t2.micro --user-data file://installapp.sh --count 3 --placement AvailabilityZone=us-west-2a
echo -e "New Instance creation completed"
...
echo -e "\e[1mRegistering newly created instances with the load balancer \e[0m"
...
aws autoscaling create-launch-configuration --launch-configuration-name webserver --image-id $1 --key-name awskey --instance-type t2.micro --user-data file://sample.sh
